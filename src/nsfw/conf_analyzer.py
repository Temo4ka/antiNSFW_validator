    ...
    for transform_name, transform_params in transform_config.items():
        if transform_name == "RandomResizedCrop":
            ...
            transform_list.append(
                T.RandomHorizontalFlip(p=transform_params.get("p", 0.5))
            )
            ...
            transform_list.append(
                T.GaussianBlur(kernel_size=transform_params["kernel_size"])
            )
            ...
            transform_list.append(
                T.Normalize(mean=transform_params["mean"], std=transform_params["std"])
            )
